...
- **`Transaction.transactionDate` / `DividendHistory.paymentDate` の型**。`DateTime` のまま据え置く。読み取り時に `toDateKey` を通しており、実害が出ていないため
- **既存データの遡及修正**。ローカル DB の 1 日ずれたレコードは直さない。本番（TiDB）へは移送せず取り直す（ADR 0013）
- **場中価格の履歴（`PriceHistory`）の集計単位**。[週次・月次の間引き](../../src/app/api/prices/history/[stockId]/route.ts)はローカル時刻で年・月を取っているままで、UTC では区切りが 9 時間ずれる。表示用の間引きで永続化しないため今回は触っていない
